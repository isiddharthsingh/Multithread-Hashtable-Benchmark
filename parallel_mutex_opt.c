...
pthread_mutex_t lock[NUM_BUCKETS];          // Lock for each bucket to manage writes
pthread_mutex_t reader_lock[NUM_BUCKETS];    // Additional lock to manage readers
...
// This function supports parallel insertions across different buckets.
void insert(int key, int val) {
    int i = key % NUM_BUCKETS;
    bucket_entry *e = (bucket_entry *) malloc(sizeof(bucket_entry));
    if (!e) panic("No memory to allocate bucket!");

    // Lock only the specific bucket to allow parallel inserts into different buckets
    pthread_mutex_lock(&lock[i]); // Lock the bucket for writing
    e->next = table[i];
    e->key = key;
    e->val = val;
    table[i] = e;
